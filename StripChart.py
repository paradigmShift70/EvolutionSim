...
class StripChart( tk.Frame ):
   def __init__( self, parent, scale, historySize, trackColors, *args, **opts ):
      ...
      self._canvas = tk.Canvas( self, height=self._chartHeight + 17, width=self._chartLength, background='black' )
      ...
      self._trackColor.update( { 'tick':'white', 'tickline':'white', 'ticklabel':'white' } )
   ...
   def plotValues( self, **vals ):
      for trackName, trackHistory in self._trackHist.items():
         ...
         try:
            ...
            objId = self._canvas.create_line( x, y, x+1, y, fill=color, width=3, tags=trackName )
...
if __name__ == '__main__':
   ...
   def plotNextVals( ):
      global val_A, val_B, val_C, tickCount, iterations
